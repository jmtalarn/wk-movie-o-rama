(function(angular) {
  ...
  movies_wkmor.controller('moviesCatalogController', ['$scope', '$element', '$attrs', '$transclude', '$http', '$routeParams', 'movies', '$location',

    function($scope, $element, $attrs, $transclude, $http, $routeParams, movies, $location) {
      $scope.loading = true;
      $scope.results = [];
      movies.list().then(
        function(res) {
      ...
      $scope.go = function(id) {
        $location.path("/app/dashboard/movie/" + id);
      };
    }
  ]);
  movies_wkmor.controller('movieController', ['$scope', '$element', '$attrs', '$transclude', '$http', '$routeParams', 'movies',

    function($scope, $element, $attrs, $transclude, $http, $routeParams, movies) {
      $scope.loading = true;
      //$scope.movie = {};

      movies.get($routeParams.id).then(
        function(res) {
  ...
  movies_wkmor.directive('movieCover', function() {
    return {
      ...
      link: function(scope, element, attrs) {
        attrs.$observe('movieCover', function(url) {
